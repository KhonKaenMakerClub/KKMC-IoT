...
import com.numberx.kkmctimer.alarms.AlarmStateManager;
import com.numberx.kkmctimer.provider.Alarm;
import com.numberx.kkmctimer.provider.ClockContract;
import com.numberx.kkmctimer.stopwatch.StopwatchService;
import com.numberx.kkmctimer.stopwatch.Stopwatches;
import com.numberx.kkmctimer.timer.TimerFragment;
import com.numberx.kkmctimer.timer.TimerObj;
import com.numberx.kkmctimer.timer.Timers;
import com.numberx.kkmctimer.topic.TopicActivity;
...
public class DeskClock extends Activity implements LabelDialogFragment.TimerLabelDialogHandler,
    ...
    private KKMCTimerSync mKKMCTimerBackgroundCheck;
    ...
    // Handler and runnables for setting the action bar title based on KKMCTimer server status
    ...
    final Runnable lightUpPiOffline = new Runnable() {
        public void run() {
            ...
            mActionBar.setTitle(
                    Html.fromHtml("KKMC IoT <font color='#" + offlineRed + "'>OFFLINE</font>"));
        }
    };
    final Runnable lightUpPiOnline = new Runnable() {
        public void run() {
            mActionBar.setTitle("KKMC IoT ONLINE");
        }
    };
    ...
    protected void onCreate(Bundle icicle) {
        ...
        NetpieServiceHelper.Init(this,
                prefs.getString(SettingsActivity.KEY_NETPIE_SERVER,""),
                prefs.getString(SettingsActivity.KEY_NETPIE_APPID,""),
                prefs.getString(SettingsActivity.KEY_NETPIE_REST,""));
        // Instantiate the KKMCTimerSync and start checking if the server is up
        String correctString = "android:switcher:" + mViewPager.getId() + ":" + ALARM_TAB_INDEX;
        mKKMCTimerBackgroundCheck = new KKMCTimerSync(this, correctString);
        mKKMCTimerBackgroundCheck.startBackgroundServerCheck(mHandler, lightUpPiOnline, lightUpPiOffline);
    }
    ...
    public void onPause() {
        ...
        mKKMCTimerBackgroundCheck.stopBackgroundServerCheck();
