...
package com.numberx.kkmctimer.provider;
...
import com.numberx.kkmctimer.Log;
...
public final class Alarm implements Parcelable, ClockContract.AlarmsColumns {
    ...
    private static final int KKMCTIMER_ID_INDEX = 9;
    ...
    public static ContentValues createContentValues(Alarm alarm) {
        ...
        values.put(KKMCTIMER_ID, alarm.kkmctimerId);
    ...
    /**
     * Get alarm by KKMCTimer ID.
     *
     * @param contentResolver to perform the query on.
     * @param kkmctimerId ID for the KKMCTimer alarm to find.
     * @return alarm if found, null otherwise
     */
    public static Alarm getAlarmLightuppiId(ContentResolver contentResolver, long kkmctimerId) {
        Cursor cursor = contentResolver.query(
                CONTENT_URI, QUERY_COLUMNS, Alarm.KKMCTIMER_ID + "=" + kkmctimerId, null, null);
        if (cursor == null) {
            return null;
        }
    ...
    public static boolean updateAlarm(
        ...
        if (bypassTimestamp.length <= 0 || !bypassTimestamp[0]) {
            alarm.timestamp = (long) (System.currentTimeMillis() / 1000);
            if (Log.LOGV) Log.d("Alarm " + alarm.id + "/" + alarm.kkmctimerId + " new timestamp");
        }
        ContentValues values = createContentValues(alarm);
    ...
    public Alarm(Cursor c) {
        ...
        kkmctimerId = c.getLong(KKMCTIMER_ID_INDEX);
    ...
    public AlarmInstance createInstanceAfter(Calendar time) {
        ...
        result.mLightuppiId = kkmctimerId;
